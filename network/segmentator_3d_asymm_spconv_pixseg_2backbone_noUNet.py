...
class Asymm_3d_spconv(nn.Module):
    def __init__(self,
        ...
        ### us start
        self.secDown0 = ResBlock(2 * init_size, 4 * init_size, 0.2, height_pooling=True, indice_key="secDown0") # 64 -> 128
        self.secDown1 = ResBlock(4 * init_size, 8 * init_size, 0.2, height_pooling=True, indice_key="secDown1") # 128 -> 256
        self.secDown2 = ResBlock(8 * init_size, 16 * init_size, 0.2, pooling=True, height_pooling=False, indice_key="secDown2") # 256 -> 512
        self.secDown3 = ResBlock(16 * init_size, 32 * init_size, 0.2, pooling=True, height_pooling=False, indice_key="secDown3") # 512 -> 1024

        self.secUp0 = UpBlock(32 * init_size, 32 * init_size, indice_key="secUp0", up_key="secDown3") # 1024 -> 1024
        self.secUp1 = UpBlock(32 * init_size, 16 * init_size, indice_key="secUp1", up_key="secDown2")  # 1024 -> 512
        self.secUp2 = UpBlock(16 * init_size, 8 * init_size, indice_key="secUp2", up_key="secDown1")  # 512 -> 256

        self.secUp3 = UpBlock(8 * init_size, 4 * init_size, indice_key="secUp3", up_key="secDown0")  # 256 -> 128
        self.secUp3_ins_heatmap = UpBlock(8 * init_size, 4 * init_size, indice_key="secUp3_ins_heatmap",
                                          up_key="secDown0") # 256 -> 128
        self.secUp3_ins_offset = UpBlock(8 * init_size, 4 * init_size, indice_key="secUp3_ins_offset",
                                          up_key="secDown0") # 256 -> 128
        self.secUp3_ins_instmap = UpBlock(8 * init_size, 4 * init_size, indice_key="secUp3_ins_instmap",
                                          up_key="secDown0")  # 256 -> 128

        self.outconv = spconv.SparseConv3d(4 * init_size, 2 * init_size, kernel_size=1, stride=1, padding=0, indice_key="outc") # 128 -> 64
        self.outconv_ins_heatmap = spconv.SparseConv3d(4 * init_size, 2 * init_size, kernel_size=1, stride=1,
                                           padding=0, indice_key="outc_ins_heatmap") # 128 -> 64
        self.outconv_ins_offset = spconv.SparseConv3d(4 * init_size, 2 * init_size, kernel_size=1, stride=1,
                                                       padding=0, indice_key="outc_ins_offset")  # 128 -> 64
        self.outconv_ins_instmap = spconv.SparseConv3d(4 * init_size, 2 * init_size, kernel_size=1, stride=1,
                                                       padding=0, indice_key="outc_ins_instmap")  # 128 -> 64
        ### us end
        ...
        self.compress_heatmap = spconv.SubMConv3d(4 * init_size, 32, indice_key="compress_heatmap", kernel_size=3,
        ...
        self.logits_heatmap = nn.Conv2d(32, 1, 3, padding=(1, 0))
    ...
    def forward(self, voxel_features, coors, batch_size, pooled):
        ...
        up1e = self.upBlock3(scomp0, down1b) # [1, 64, 480, 360, 32]
        ### us start
        secD1c, secD1b = self.secDown0(up1e)
        secD2c, secD2b = self.secDown1(secD1c)
        secD3c, secD3b = self.secDown2(secD2c)
        secD4c, secD4b = self.secDown3(secD3c)

        secU4e = self.secUp0(secD4c, secD4b)
        secU3e = self.secUp1(secU4e, secD3b)
        secU2e = self.secUp2(secU3e, secD2b)
        ### us end

        secU1e = self.secUp3(secU2e, secD1b) # us
        secU0e = self.outconv(secU1e) # us
        up0e = self.ReconNet(secU0e) # us
        up0e = up0e.replace_feature(torch.cat((up0e.features, secU0e.features), 1)) # us
        logits = self.logits(up0e)
        logits = logits.dense()

        secU1e_ins_heatmap = self.secUp3_ins_heatmap(secU2e, secD1b)  # us
        secU0e_ins_heatmap = self.outconv_ins_heatmap(secU1e_ins_heatmap)  # us
        up0e_ins_heatmap = self.ReconNet_ins_heatmap(secU0e_ins_heatmap) # us
        up0e_ins_heatmap = up0e_ins_heatmap.replace_feature(
            torch.cat((up0e_ins_heatmap.features, secU0e_ins_heatmap.features), 1)) # us
        up0e_ins_heatmap = self.pool3d_heatmap(up0e_ins_heatmap)
        up0e_ins_heatmap = self.compress_heatmap(up0e_ins_heatmap) # us
        heatmap = up0e_ins_heatmap.dense().squeeze(-1)
        heatmap = F.pad(heatmap, (1, 1, 0, 0), mode='circular') # us
        heatmap = self.logits_heatmap(heatmap) # us

        secU1e_ins_offset = self.secUp3_ins_offset(secU2e, secD1b)  # us
        secU0e_ins_offset = self.outconv_ins_offset(secU1e_ins_offset)  # us
        up0e_ins_offset = self.ReconNet_ins_offset(secU0e_ins_offset) # us
        up0e_ins_offset = up0e_ins_offset.replace_feature(
            torch.cat((up0e_ins_offset.features, secU0e_ins_offset.features), 1)) # us
        ...
        secU1e_ins_instmap = self.secUp3_ins_instmap(secU2e, secD1b)  # us
        secU0e_ins_instmap = self.outconv_ins_instmap(secU1e_ins_instmap)  # us
        up0e_ins_instmap = self.ReconNet_ins_instmap(secU0e_ins_instmap) # us
        up0e_ins_instmap = up0e_ins_instmap.replace_feature(
            torch.cat((up0e_ins_instmap.features, secU0e_ins_instmap.features), 1)) # us
