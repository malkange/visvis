...
class SemKITTI_pt(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_encode = mask_idx[:, 0]
        mask_encode = torch.sin(mask_encode / len(segfea) * math.pi - math.pi / 2) / 2 + 1 / 2

        pts = np.concatenate((pts, np.array(torch.unsqueeze(mask_encode, dim=1))), axis=1)
        ### us end
