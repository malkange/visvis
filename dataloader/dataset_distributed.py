...
class SemKITTI_pt(data.Dataset):
    def __init__(self, data_path, cfgs, split='train', return_ref=False):
        ...
        # sequences
        if self.split == 'train':
            self.seqs = ['00', '01', '02', '03', '04', '05', '06', '07', '09']
        elif self.split == 'val':
            self.seqs = ['08', '10']
    ...
    def __getitem__(self, index):
        ...
        # pixel_coordinates2 = pixel_coordinates2 / (max_len - 1) * 2 - 1.0
            # gh divide by max length for grid-sampling (normalized pixel coordinates)
        ### gh ###


        ### gh ###
        # get sem feature / index
        ...
        segidx[segidx==255] = len(segfea)
            # segidx == 255 : no matching pts
            # indicate last index
        # print("!!!", pts.shape) : (110228, 4)
        # print("@@@", segfea_.shape) : (62, 256) 62 number of mask feature in one image
        # print("###", segidx.shape) : (3, 18042) 3 means mask1, mask2, mask3
        xyz_segfea = torch.zeros((len(pts), 3, 256))
        xyz_segfea[mask1] = segfea_[segidx].transpose(1,0)
        # print("###", mask1.shape) : (127810, )
        # print(mask1) : (true, true, ...)
        # print("@@@", segfea_[segidx].shape) : (3, 20145, 256)
        # print("###", segfea_[segidx].transpose(1,0).shape) : (20145, 3, 256)
        # xyz_segfea : (127810, 3, 256)
            #above two np array matched

        # pixfea
        imgfea = torch.zeros((256, 64, 64), dtype=torch.float32)
        imgfea[:, pix_loc[0], pix_loc[1]] = imgfea_sub.to(torch.float32)
        # print("@@@", imgfea_sub.shape) : (256, 2566)
            # features
        # print("###", pix_loc.shape) : (2, 2566)
            # [[ 0,  0,  0,  ..., 63, 63, 63],
            # [21, 22, 23,  ..., 60, 61, 62]]
        xyz_pixfea = torch.zeros((len(pts), 256), dtype=torch.float32)
        uvs = pixel_coordinates1[mask1]
        # print("!!!", pixel_coordinates1.shape) : (128309,2)
        # print("$$$", pixel_coordinates1[mask1].shape) (20612, 2)
        grid_tmp = torch.Tensor(uvs[None, :,None, :])
            # (1, 20612, 1, 2)
        xyz_pixfea_sub = torch.nn.functional.grid_sample(imgfea[None, ...], grid_tmp)[0, ..., 0].transpose(1,0)
            # sampling features from imgfea to xyz_pixfea_sub using grip_tmp
            # input = imgfea[none, ...] : (1, 256, 64, 64) = (batch, channel, height, width)
            # grid = grid_tmp : (1, 20612, 1, 2) = (batch, output height, output width, coordinate)
            # grid_sample : (1, 256, 20612, 1) = (batch, channel, output height, output width)
            # [0, ... , 0] : (256, 20612)
            # transpose(1, 0) : (20612, 256)
...
class spherical_dataset(data.Dataset):
    def __init__(self, in_dataset, cfgs, ignore_label=0, fixed_volume_space=True, use_aug=True):
        ...
        self.panoptic_proc = PanopticLabelGenerator(self.grid_size, sigma=cfgs['dataset']['gt_generator']['sigma'],polar=True)
    ...
    def __getitem__(self, index):
        ...
        # return_dict['rotate_deg'] = rotate_deg # gh
        # if type(labels) == np.ndarray and type(insts) == np.ndarray: #gh
