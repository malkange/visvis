    ...
    def __getitem__(self, index):
        ...
        for i in range(len(pts)):
            if mask1[i] == True:
                ...
                count += 1
                # if mask_idx[i][0] == 0 and mask_idx[i][1] == 0 and mask_idx[i][2] == 0: allzero += 1 # debug

        # tmp = torch.zeros((len(pts)))
        # for j in range(len(pts)):
        #    tmp[j] = mask_idx[j][0] + mask_idx[j][1] + mask_idx[j][2]
        # print(np.count_nonzero(mask1), tmp.nonzero(as_tuple = True)[0].size(dim = 0) + allzero)
