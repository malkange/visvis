...
class SemKITTI_pt(data.Dataset):
    def __init__(self, data_path, cfgs, split='train', return_ref=False):
        ...
        # gh
        self.root_sam = data_path.replace('SemKITTI',
    ...
    def __getitem__(self, index):
        ...
        mask_idx = torch.zeros((len(pts)), 3)
        count = 0
        for i in range(len(pts)):
            if mask1[i] == True:
                mask_idx[i] = segidx_[:, count]
                count += 1

        mask_center = torch.zeros((len(segfea) + 1), 3)
        mask_count = torch.zeros((len(segfea) + 1))
        for i in range(len(pts)):
            for j in range(1, len(mask_center)):
                if mask_idx[i][0] == j:
                    mask_center[j][0] += pts[i][0]
                    mask_center[j][1] += pts[i][1]
                    mask_center[j][2] += pts[i][2]
                    mask_count[j] += 1
                    break
        mask_center[1:] /= mask_count[1:]
        xyz_center = torch.zeros((len(pts)), 3)
        for i in range(len(pts)):
            xyz_center[i] = mask_center[mask_idx[i][0]]

        pts = np.concatenate((pts, np.array(torch.unsqueeze(mask_idx[:, 0], dim=1))), axis=1)
        pts = np.concatenate((pts, np.array(xyz_center)), axis=1)
        ### us end
    ...
    def __getitem__(self, index):
        ...
        if self.rotate_aug:
            ...
            for i in range(len(mask_idx)):
                if mask_idx != 0:
                    xyz_center[i][0] = xyz_center[i][0] + x_offset
                    xyz_center[i][1] = xyz_center[i][1] + y_offset
                    xyz_center[i, :2] = np.dot(xyz_center[i, :2], j)
            ### us end
        ...
        if self.flip_aug:
            ...
            if flip_type == 1:
                xyz[:, 0] = -xyz[:, 0]
                xyz_center[:, 0] = -xyz_center[:, 0] # us
            elif flip_type == 2:
                xyz[:, 1] = -xyz[:, 1]
                xyz_center[:, 1] = -xyz_center[:, 1] # us

        # 转化成极坐标系
        xyz_pol = cart2polar(xyz)
        xyz_center_pol = cart2polar(xyz_center)
        ...
        return_fea = np.concatenate((return_xyz, xyz_center_pol, xyz_center[:, :2]), axis=1) # us
