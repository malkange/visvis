...
class PanopticEval:
  ...
  def getPQ(self):
    ...
    P_all = self.pan_tp.astype(np.double) / np.maximum(
        self.pan_tp.astype(np.double) + self.pan_fp.astype(np.double), self.eps)
    R_all = self.pan_tp.astype(np.double) / np.maximum(
        self.pan_tp.astype(np.double) + self.pan_fn.astype(np.double), self.eps)

    # tp_all = self.pan_tp.astype(np.double)
    # fp_all = self.pan_fp.astype(np.double)
    # fn_all = self.pan_fn.astype(np.double)
    ...
    return PQ, SQ, RQ, pq_all, sq_all, rq_all, P_all, R_all
