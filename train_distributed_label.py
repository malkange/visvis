...
from dataloader.dataset_distributed_label import collate_fn_BEV, Nuscenes_pt, spherical_dataset, collate_dataset_info, SemKITTI_pt # us
...
from dataloader.pc_dataset import get_SemKITTI_label_name # us
# gh
...
def main():
    ...
    parser.add_argument('-m', '--labelmap', default='configs/label_mapping/semantic-kitti.yaml') # us
    parser.add_argument('-s', '--save_dir', default='/home/data/SemanticKitti/dataset/sequences/08/predictions_base/')  # us
    ...
    logger.info(cfgs)
    ### us start
    with open(args.labelmap, 'r') as stream:
        ...
        if (epoch >= 0): # us > -> >=
            ...
            with torch.no_grad():
                for i_iter_val, val_dict in enumerate(val_dataset_loader):
                    ...
                    predict_labels_out = predict_labels_sem # us
                    predict_labels_out[predict_labels_out == 20] = 0 # us
                    ...
                    for count, i_val_grid in enumerate(val_grid):
                        ...
                        if args.local_rank < 1:
                            ...
                            ### us start
                            inv_labels = np.vectorize(inv_learning_map.__getitem__)(predict_labels_out[count, val_grid[count][:, 0], val_grid[count][:, 1], val_grid[count][:, 2]])
                            inv_labels = inv_labels.astype('uint32')
                            outputPath = args.save_dir + str(i_iter_val).zfill(6) + '.label'
