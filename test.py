...
def main():
    ...
    with torch.no_grad():
        for i_iter_val, test_dict in enumerate(test_dataset_loader):
            ...
            test_dict['camera_channel'] = torch.from_numpy(test_dict['camera_channel']).cuda()  # todo
            
            test_dict['segfea'] = [i.cuda() for i in test_dict['segfea']]
                    test_dict['pixfea'] = [i.cuda() for i in test_dict['pixfea']]
                    test_dict['segscore'] = [i.cuda() for i in test_dict['segscore']]
                    
            predict_labels, center, offset, instmap, _, cam, _ = my_model(test_dict)
